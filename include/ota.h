...
namespace OTA {
  ...
  inline bool connectAnyWifi()
  {
    ...
    for (int i = 0; i < n; i++) {
      ...
      // Only try open networks (no password)
      if (auth == WIFI_AUTH_OPEN) {
        Serial.printf("Trying open network: %s\n", ssid.c_str());

        WiFi.begin(ssid.c_str(), cmco_password);     // no password
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          break;  // stop after first successful open network
        } else {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("No open WiFi networks connected");
      return false;
    }
    return true;
  }
